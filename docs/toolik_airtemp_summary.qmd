...
toolik_weather <- toolik_weather %>% 
  janitor::clean_names() # What does this do? Say it in words.
...
First, read in chlorophyll data
```{r}
# Reading in data using hear::hear
...
# Want to replace missing values with NA
toolik_chlorophyll$active_chl_a_ug_l <-
  na_if(toolik_chlorophyll$active_chl_a_ug_l, ".")

# Convert chlorophyll data into numeric
toolik_chlorophyll$active_chl_a_ug_l <- 
  as.numeric(toolik_chlorophyll$active_chl_a_ug_l)

# Want to replace missing values with NA
toolik_chlorophyll$pprod_mg_available_c_m3_d <-
  na_if(toolik_chlorophyll$pprod_mg_available_c_m3_d, ".")

# Convert chlorophyll data into numeric
toolik_chlorophyll$pprod_mg_available_c_m3_d<- 
  as.numeric(toolik_chlorophyll$pprod_mg_available_c_m3_d)

# PLOT IT !
toolik_chlorophyll_plot <- ggplot(data= toolik_chlorophyll,
       aes(x= depth, y= active_chl_a_ug_l)) +
  geom_point() +
  theme_minimal() +
